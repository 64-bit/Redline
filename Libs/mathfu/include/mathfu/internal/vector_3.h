...
class Vector<T, 3> {
 public:
  ...
  inline const Vector<T, 3> Saturated() const
  {
	  return Max(Min(*this, Vector<T, 3>(1.0f, 1.0f, 1.0f)), Vector<T, 3>(0.0f, 0.0f, 0.0f));
  }
  ...
  inline Vector<T, 3> Abs() const { return AbsHelper(*this); }
